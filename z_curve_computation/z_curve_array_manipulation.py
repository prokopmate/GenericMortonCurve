...
def z_index_array_manipulation(variables, coord_type, index_type, var_count):
    ...
    if variables_transposed.dtype.byteorder != ">":
    ...
    return index_type(np.squeeze(np.frombuffer(packed_index_bytes, dtype=np.dtype(index_type).newbyteorder('>'))))
...
def variables_array_manipulation(z_index_arr, coord_type, index_type, var_count):
    ...
    if z_index_arr_idx_type.dtype.byteorder != ">":
