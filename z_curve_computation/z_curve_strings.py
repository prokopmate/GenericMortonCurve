...
def z_index_string_manipulation(variables_array, coord_type, index_type, var_count):
    bigger_endian_index_type = np.dtype(index_type).newbyteorder('>')  # ensures correct order for conversion
    ...
    at_least_2d_vars = np.atleast_2d(variables_array)
    z_index_arr = np.empty(len(at_least_2d_vars), dtype=bigger_endian_index_type)
    for index, variables in enumerate(at_least_2d_vars):
        ...
        z_index_arr[index] = np.frombuffer(result_bytes, dtype=bigger_endian_index_type)[0]
    return np.squeeze(z_index_arr)


def variables_string_manipulation(z_index_arr, coord_type, index_type, var_count):
    bigger_endian_coord_type = np.dtype(coord_type).newbyteorder('>')  # ensures correct order for conversion
    coordinate_bits_count = np.iinfo(coord_type).bits
    index_bits_count = np.iinfo(index_type).bits
    final_string_filled_len = var_count * coordinate_bits_count
    z_indexes_at_least_1d=np.atleast_1d(index_type(z_index_arr))
    variables_arr = np.empty((len(z_indexes_at_least_1d), var_count), dtype=bigger_endian_coord_type)
    for index, z_index in enumerate(z_indexes_at_least_1d):
        ...
        variables_arr[index] = np.frombuffer(result_bytes, dtype=bigger_endian_coord_type)
